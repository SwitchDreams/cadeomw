...
const ListCourses: React.FC = () => {
  ...
  function handleSearchCourse(event:FormEvent<HTMLFormElement>): void {
    ...
    console.log(event);
    apiCourses.get<CourseInfos>(`courses/?search=${searchCourse}&format=json`).then((response) => {
       console.log(response.data);
       setCourses(response.data);
    });    
    setSearchCourse('');
    ...
    <>
      ...
      <Form>
        <form onSubmit={handleSearchCourse}>
          ...
          placeholder="Digite o nome do curso"></input>  
      ...
      <Courses>
        {courses.results.map(course => (
          <a key={course.code} href={`course/${course.code}`}>
            <div>
        ...
        <div className="actions">
            <button disabled = {courses.previous == null} onClick={() => handlePagination(courses.previous)}>Anterior</button>
            <button disabled = {courses.next == null} onClick={() => handlePagination(courses.next)}>Próximo</button>
        </div>
